...
```{r}
write_tsv(iris_raw, "iris_data.tsv")
```
...
iris_raw %>% 
  group_by(species) %>% 
  summarise(mean = mean(sepal_length), median = median(sepal_length), minimum = min(sepal_length), max= max(sepal_length))
...
iris_for_ggplot + facet_wrap(~ species)
```
...
```{r}
iris_long <- iris_raw %>% 
  pivot_longer(where(is.numeric), names_to = "Measurement _type", values_to = "Measurement")
...
iris_long2 <- iris_raw %>%
  ...
  pivot_longer(where(is.numeric), names_to = "Measurement_type", values_to = "Measurement")
...
exprs_assay <- read_excel("EH2011.xlsx", "exprs")
...
exprs_assay_long <- exprs_assay %>% 
   pivot_longer(where(is.numeric), names_to = "Sample_Names", values_to = "Expression")
...
```{r}
hist_plot <- ggplot(exprs_assay_long, aes(x = Expression, fill = gene)) +
         geom_histogram()
hist_plot + facet_wrap(~gene)
```
...
```{r}
excel_sheets(   )
sample_info <- read_excel("EH2011.xlsx", "sample_info")
sample_info
```
...
sample_info <- sample_info %>%
    mutate(across(as.factor(c(grade, T, N, M))))
sample_info
```
