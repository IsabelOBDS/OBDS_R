...
DO NOT INSTALL PACKAGES!!!! TALK TO ADMIN
...
```{r}
PhredQuality(0:40)
# this is the quality string score - accuracy of sequencing 
```
...
```{r, eval=FALSE}
vignette(topic = "Biostrings2Classes", package = "Biostrings")
```
...
```{r}
truseq_adapters <- readDNAStringSet("/project/obds/imorgan/2_r/projects/bioconductor/TruSeq3-PE-2.fa")
...
#Tells you what genome files are available (can be used as references to align reads)
```
...
gtf_data$ #to extract the metadata you are interested in
```
...
print(unique(na.omit(gtf_ACTB$transcript_id)))

#Other way to solve it! 
length(subset(gtf_ACTB, type == "transcript")$transcript_id)
...
lengths(gtf_ACTB_transcripts) # This tells you how many exons are in each transcripts
```
...
https://www.ensembl.org/biomart/martview/15cb73e3aa73a93114b2089e5f03136a
...
from the object.
Each column is an attribute (all things you can query in the database).
...
if you have gene id you can find the chromosome start position etc. (some columns are keys but not certain)
...
```{r}
AnnotationDbi::select(org.Hs.eg.db, keys = human_symbols, keytype = "SYMBOL", columns = c("ENSEMBL", "ENTREZID")   )
```
...
```{r}
mapIds(org.Hs.eg.db, keys = human_symbols, column = "ENSEMBL", keytype = "SYMBOL") # further argument you can add is  ',multiVals = "CharacterList)" but this gives a list rather than vector 
```
...
```{r}
assay_counts <- read.csv("counts.csv", row.names = "X")
...
```{r}
sample_metadata <- read.csv("sample_metadata.csv", row.names = "X")
head(sample_metadata)
```

```{r}
gene_metadata <- read.csv("gene_metadata.csv", row.names = "X")
head(gene_metadata)
```
