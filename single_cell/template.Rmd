...
```{r}
class(read10x_data)
#not a seurat obj initially - this is a dgCMatrix
```
...
# all of the '.' are actually zero. saves space to store it this way.
#some names in the first column, uses gene names symbol and then puts .number after duplicates (non-unique values - not good becuase gene symbol looses it's meaning and it is now hard to search for - you would not merge this as they are physically different. produces the same protein but they are in different parts of the genome or place on chromosome)
# gene.column = 1 would show gene ensamble. 
```
...
> Answer:
> 33538 features/genes  5155 cells/barcodes
...
```{r}
#project is one dataset - only need to use it if you need to merge cells. 
seurat_object <- CreateSeuratObject(
    counts = read10x_data,
    project = "pbmc5k", #this is an arbitray itdentifier of the project
    min.cells = 3, #these are arbitrary values that we have selected, seurat probably uses a default value
    min.features = 200 ##these are arbitrary values that we have selected, seurat probably uses a default value
...
```{r}
dim(read10x_data   ) - dim(seurat_object) # figures out how many barcodes/features have been removed. - removes empty droplets but not yet doubles? and filters out genes that are undetected. 
```
...
```{r}
seurat_object[[]] #gives you the wqhole table of the metadata 
...
```{r}
FetchData(object = seurat_object, vars = c("LYZ", "nCount_RNA"), slot = "data"   )
```
...
normally remove doublets before doing this. 
...
```{r}
seurat_after_qc <- NormalizeData(seurat_after_qc, normalization.method = "LogNormalize") # scale.factor = counter per 10000. It is arbitrary. just changes where the decimal point is. 
#in RNA assay it replaces the data compnent 
...
```{r}
GetAssayData(seurat_after_qc, slot = "counts") # RNA assay has the original counts and the lognormalised counts.

plot_data_norm <- FetchData(seurat_after_qc, var = "LYZ", slot = "counts")
plot_data_log <- FetchData(seurat_after_qc, var = "LYZ", slot = "data")
ggplot_lyz_raw <- ggplot(plot_data_norm, aes(x = LYZ)) +
    geom_histogram(   ) +
    coord_cartesian(ylim = c(0, 500)) +
    cowplot::theme_cowplot()
ggplot_lyz_normalised <- ggplot(plot_data_log, aes(x = LYZ) ) +
    geom_histogram(   ) +
    coord_cartesian(ylim = c(0, 500)) +
    cowplot::theme_cowplot()
...
> Answer: "IGLC3"    "IGLC2"    "FCER1A"   "S100A9"   "S100A8"   "IGKC"    
 "HLA-DQA1" "GNLY"     "C1QA"     "LYZ" (highest variance relative to its mean first)
> 
...
seurat_after_qc <- ScaleData(seurat_after_qc, vars.to.regress = c("nCount_RNA", "percent_mt")
    # correct for library size and for those expressing high mitochondrial genes 
    #fit a linear model to each gene, any trend on gene to library size, vells and brought down to the trent 
    #more mt content you might have lower reads becasue there are so many mt, this evens out the ratio. 
...
pca_data <- FetchData(seurat_after_qc,   var = c("PC_1", "PC_2"))
head(pca_data)
```
...
saveRDS(seurat_after_qc, file = "Seurat_obj_after_Umap_Feb2023.RDS")
```
...
seurat_after_qc <- FindNeighbors(seurat_after_qc, 
                                # reduction = "pca", 
                                 dims = 1:20, 
                                 k.param = 20   )
#the more cells you have in the dataset the more k you should use. The larger the K value, you wont be able to find smaller clusters (larger K, greater change of finding more neighbours) 100,000 cells maybe 30-50 k. lower k, higher resolution? 
...
seurat_after_qc <- FindClusters(seurat_after_qc, 
                                graph.name = "RNA_snn",
                                resolution = res, #computing different resolutions 0.5-1 in 0.1 increments               
                                algorithm = 1 # community detection algorithm (louvain is the default)
                                )
#run clusttree to findout the best resolution for your data
```
```{r}
library(clustree)
...
seurat_markers_all <- FindAllMarkers(object = seurat_after_qc,
...
seurat_markers_all %>% filter(cluster == 3) %>%
  ...
  slice_head(n = 10) %>% 
  pull(gene) -> seurat_markers_top_ten_c3

#slice_head tidyverse takes a number or %
```
...
FeaturePlot(seurat_after_qc, 
            features = seurat_markers_top_ten_c3[1:4],
            label = TRUE
)
...
markers_top10_clusters <-
  seurat_markers_all %>% group_by(cluster) %>% filter(p_val_adj < 0.05) %>% arrange(desc((avg_log2FC))) %>%
  slice_head(n = 10) %>% 
  pull(gene)
