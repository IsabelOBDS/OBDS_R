...
```{r}
iris_features <- iris %>% 
  select(contains('width') | contains('length')) %>% 
  as.matrix()
head(iris_features)
class(iris_features)
```
...
```{r}
pca_iris_species_df <-
  cbind(Species = iris$Species, pca_iris_dataframe)
pca_iris_species <-
  ggplot(pca_iris_species_df, aes(x = PC1, y = PC2, color = Species)) +
  geom_point()
...
```{r}
ggplot(pca_iris_dataframe, aes(x = PC1, y = PC2, colour = iris$Petal.Length)) +
  geom_point()  
...
```{r}
str(pca_iris)
#to get the variance of the data, you can square the standard deviation. 
...
```{r}
variance_dataframe <- data.frame(explained_variance_ratio, PC = as.character(c(1:4))
...
umap_iris_layout <- umap_iris$layout 
colnames(umap_iris_layout)[1] ="x"
colnames(umap_iris_layout)[2] ="y"
...
```{r}
str(umap_iris_layout)
```

- Visualise the UMAP projection using `ggplot2::geom_point()`.

```{r}
umap_iris_layout_dataframe <- data.frame(umap_iris_layout)
umap_iris_dataframe <- ggplot(umap_iris_layout_dataframe, aes(x = x, y = y)) +
  geom_point()
...
```{r}
umap_iris_species_df <-
  cbind(Species = iris$Species, umap_iris_layout_dataframe)

head(umap_iris_species_df)
```

```{r}
umap_iris_species <- ggplot(umap_iris_species_df, aes(x = x, y = y, colour = Species)) +
  geom_point()
...
# k argument means that you choose the number of clusters # h, you choose the height on the tree plot to show where you cut the tree. 
...
plot_average <- ggplot(iris_clusters_dataframe, aes(x = Petal.Length, y = Petal.Width, color = hclust_average)    ) +
  geom_point() + theme_bw()
  
  
plot_complete <- ggplot(iris_clusters_dataframe, aes(x = Petal.Length, y = Petal.Width, color = hclust_complete)    ) +
  geom_point() + theme_bw()
  
  
plot_single <- ggplot(iris_clusters_dataframe, aes(x = Petal.Length, y = Petal.Width, color = hclust_single)    ) +
  geom_point() + theme_bw()
  
  
plot_ward <- ggplot(iris_clusters_dataframe, aes(x = Petal.Length, y = Petal.Width, color = hclust_ward)    ) +
  geom_point() + theme_bw()
  
  
cowplot::plot_grid(
  plot_average, plot_complete,
    plot_single, plot_ward
...
```{r}
#eps is the epsilon value which changes the distance between data points to find clusters. smaller the value, the calculation will only take data points close to eachother to make clusters, higher values will include more datapoints futher away in a cluster. 
