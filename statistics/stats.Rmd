...
```{r}
hist(x1, breaks = 100, xlabs = NULL)

df <- data.frame(x1)
histNorm <- ggplot(df) +
  geom_histogram(aes(x = x1), binwidth = 1, colour = 'white')
```
...
ggplot(df) +
  geom_histogram(aes(x = x1), binwidth = 1, colour = 'white') +
  geom_vline(xintercept = c(meanx1, medianx1, meanx1 + SDx1, meanx1 - SDx1, medianx1 + absMedian, medianx1 - absMedian), 
...
ggplot(df2) +
  geom_histogram(aes(x = x2), binwidth = 1, colour = 'white') +
  geom_vline(xintercept = c(meanx2, medianx2, meanx2 + SDx2, meanx2 - SDx2, medianx2 + absMedian, medianx2 - absMedian), 
             colour = c('red', 'red', 'blue', 'blue', 'blue', 'blue'), 
             linetype = c(1, 2, 1, 1, 2, 2)) +
...
```{r}
plot2 <-  ggplot(data.frame(x = c(-5, 5)), aes(x = x )) +
  stat_function(fun = qnorm)

# can also be drawn like this 
x1 <- seq(from = -5, to = 5, by = 0.1)
p1 <- qnorm(x1, mean = 0, sd = 1)
plot(x = x1, y = p1)
```
...
Given an Excel file that contains a matrix of log-normalised counts (`logcounts`) and experimental metadata (`cell_info`), test each gene (i.e., row) in the matrix for differential expression between the two experimental groups.
...
Given the list of genes (Ensembl gene identifiers) that your identified as differentially expressed in the previous exercise, and a list of gene sets (`go_db`), test each gene set for over-representation of differentially expressed genes.
